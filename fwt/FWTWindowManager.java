...
public class FWTWindowManager implements Runnable {
    ...
    /**
     * Whether FWT input events are single-threaded or multi-threaded.
     * <br> * <b>Single Threaded</b> handles input calls immediately.
     * <br> * <b>Multi-threaded</b> sends input calls to 'FWTInputThread'.
     */
    boolean singleThreadInputEvents = true;
    ...
    public synchronized void setActiveWindow(FWTWindow win) {
        ...
        try {
            if (activeWindow != null && !sameWin) activeWindow.lostFocus();
        } catch (Exception ex) {
            FWTController.error("Error on Lost Focus: " + ex.getMessage());
        }
    ...
    /**
     * Creates a new empty window manager using single-threaded design.
    ...
    /**
     * Creates a new empty window manager and whether the manager uses single-threaded inputs.
     * DO NOT USE. Create a new window manager through FWTController.createWindowManager().
     */
    protected FWTWindowManager(String name, boolean singleThreaded) {
        this();
        this.name = name;
        this.singleThreadInputEvents = singleThreaded;

        // IF multy-threaded, start input thread
        if (!singleThreadInputEvents)
            startInputThread();
    }
    ...
    /** Refreshes all windows and reloads their XML configurations. */
    ...
    public synchronized void render() {
        try {
            ...
            FWTController.error("FWTManager: Error processing FWTWindow.");
                    ...
                    if (win.getLayer() == ii)
                        if (win.isEnabled() && !win.isHidden() && win.contains(mX, mY)) {
                            ...
                            if (activeWindow == null || (!blocking || activeWindow.getID() == win.getID())) {
                                ...
                                if (singleThreadInputEvents)
                                    win.touchDown(mX, mY, pointer, button);
                                else
    ...
    public boolean touchUp(int mX, int mY, int pointer, int button) {
        ...
        try {
            ...
            if (dragComponent != null) {
                        ...
                        if (win.getLayer() == ii)
                            if (win.isEnabled() && !win.isHidden() && win.contains(mX, mY)) {
                                ...
                                if (activeWindow == null || (!blocking || activeWindow.getID() == win.getID())) {
                                    ...
                                    if (singleThreadInputEvents) {
                                        win.dragNDrop(mX, mY, pointer, button, draggedComp);
                                        draggedComp.dragRelease(mX, mY, pointer);
                                    } else {
                ...
                // DRAG RELEASE event on the dragged component
                if (singleThreadInputEvents)
                    draggedComp.dragRelease(mX, mY, pointer);
                else
                    addInputEvent(new FWTTouchEvent(draggedComp, FWTEventType.DRAG_RELEASE, mX, mY, pointer, button));
                ...
                if (mouseComp.isEnabled()) {
                    ...
                    if (singleThreadInputEvents) {
                        ...
                        if (mouseComp.getParentWindow() != null)
                            mouseComp.getParentWindow().touchUp(mX, mY, pointer, button, mouseComp);
                        else // ELSE is window
                            mouseComp.touchUp(mX, mY, pointer, button, mouseComp);
                    } else {
                        // Add touch-up event to parent window while indicating the touch-down receiver
                        if (mouseComp.getParentWindow() != null)
                ...
                for (FWTWindow win : windows) {
                    ...
                    if (win.getLayer() == ii && win.isEnabled() && !win.isHidden() && win.contains(mX, mY)) {
                        ...
                        if (activeWindow == null || (!blocking || activeWindow.getID() == win.getID())) {
                            ...
                            // IF not the same window as previous
    ...
    public boolean touchCancelled(int mX, int y, int pointer, int button) {
        return false;
    }
        ...
        try {
            for (FWTWindow win : windows)
                if (activeWindow != null && activeWindow.getID() == win.getID() && activeWindow.isEnabled()) {
                    ...
                    if (singleThreadInputEvents)
                        return win.scrolled(amountX, amountY);
                    else {
    ...
    public boolean keyDown(int keycode) {
        try {
            ...
            if (activeWindow != null && activeWindow.isEnabled()) {
                ...
                if (singleThreadInputEvents)
                    return activeWindow.keyDown(keycode);
                else {
    ...
    public boolean keyUp(int keycode) {
        try {
            ...
            if (activeWindow != null && activeWindow.isEnabled()) {
                // Handle KEY UP Event
                if (singleThreadInputEvents)
                    return activeWindow.keyUp(keycode);
                else {
    ...
    public boolean keyTyped(char keychar) {
        try {
            ...
            if (activeWindow != null && activeWindow.isEnabled()) {
                ...
                if (singleThreadInputEvents)
                    return activeWindow.keyTyped(keychar);
                else {
        ...
        switch (event.type) {
            case MOUSE_MOVED: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'mouse-moved' event for component: " + tEvent.target.getName());
                }
            }
            break;
            ...
            case ON_ENTER: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'enter' event for component: " + iEvent.target.getName());
                }
            }
            break;
            ...
            case ON_EXIT: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'exit' event for component: " + iEvent.target.getName());
                }
            }
            break;
            ...
            case WINDOW_GAIN_FOCUS: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'gain-focus' event for component: " + iEvent.target.getName());
                }
            }
            break;
            ...
            case WINDOW_LOST_FOCUS: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'lost-focus' event for component: " + iEvent.target.getName());
                }
            }
            break;
            ...
            case SCROLLED: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'scrolled' event for component: " + sEvent.target.getName());
                }
            }
            break;
            ...
            case TOUCH_UP: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'touch-up' event for component: " + tEvent.target.getName());
                }
            }
            break;
            ...
            case TOUCH_DOWN: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'touch-down' event for component: " + tEvent.target.getName());
                }
            }
            break;
            ...
            case DRAG_RELEASE: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'drag-release' event for component: " + tEvent.target.getName());
                }
            }
            break;
            ...
            case TOUCH_DRAGGED: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'touch-dragged' event for component: " + tEvent.target.getName());
                }
            }
            break;
            ...
            case DRAG_N_DROP: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'drag-N-drop' event for component: " + dEvent.target.getName());
                }
            }
            break;
            ...
            case KEY_DOWN: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'key-down' event for component: " + kEvent.target.getName());
                }
            }
            break;
            ...
            case KEY_UP: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'key-up' event for component: " + kEvent.target.getName());
                }
            }
            break;
            ...
            case KEY_TYPED: {
                ...
                try {
                    ...
                    ex.printStackTrace();
                    FWTController.error("Error processing 'key-typed' event for component: " + kEvent.target.getName());
                }
            }
            break;
